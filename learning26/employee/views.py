...
def createLibrary(request):
        ...
        if request.method == "POST":
            form = LibraryForm(request.POST) #csrftoken,form alll fileds data
            form.save() #create.. insert into table 
            return HttpResponse("LIBRARY CREATED...")
        else:
            form = LibraryForm()
            return render(request,"employee/createLibrary.html",{"form":form})

def createEvent(request):
    if request.method == "POST":
        form = EventForm(request.POST) #csrftoken,form alll fileds data
        form.save() #create.. insert into table 
